...
type responseStruct struct {
	...
	IsUserPaidTier bool   `json:"is_user_on_paid_tier"`
}
type requestStruct struct {
	Key string `json:"key"`
}

func returnTheJsonResponseonError(message string, statusCode int64, success, isUseronPaidTier bool, w http.ResponseWriter) {
	http.Error(w, message, int(statusCode))
	response := responseStruct{Message: message, Status_code: statusCode, Success: success, IsUserPaidTier: isUseronPaidTier}
...
func CheckIfKeyIsValid(os_env_key []byte) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			returnTheJsonResponseonError("Invalid request method", http.StatusBadRequest, false, false, w)
			return
		}
		...
		if err != nil {
			...
			returnTheJsonResponseonError(message, statusCode, false, false, w)
			return
		}
		if request.Key == "" {
			returnTheJsonResponseonError("key value can't be empty", http.StatusBadRequest, false, false, w)
			return
		}
